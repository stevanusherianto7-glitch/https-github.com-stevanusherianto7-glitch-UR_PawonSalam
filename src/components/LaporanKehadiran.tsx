    ...
    <div className="p-6">
      <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_2px_15px_-5px_rgba(0,0,0,0.05)]">
        <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-pink-50 rounded-t-lg">
          <h2 className="text-2xl font-bold text-slate-700">Laporan Kehadiran</h2>
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2">
              <span className="text-gray-600 text-sm">Tanggal :</span>
              <div className="bg-white border border-gray-300 text-gray-700 px-3 py-2 rounded-md text-sm">
        ...
        <div className="px-6 pt-4 border-b border-gray-200">
          <div className="flex space-x-6">
            <button className="pb-2 border-b-2 border-transparent text-gray-500 hover:text-gray-700 font-medium">
              Per Hari
            </button>
            <button className="pb-2 border-b-2 border-pink-500 text-pink-600 font-medium flex items-center space-x-1">
              <span>Per Karyawan</span>
              <span className="bg-yellow-400 text-yellow-900 text-[10px] font-bold px-1.5 py-0.5 rounded">BETA</span>
            </button>
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100">
                {[
                  { id: 1, tanggal: '6 Mei 2026', karyawanHadir: '2' },
                ].map((item, index) => (
                  <tr key={item.id} className={`hover:bg-gray-50 transition-colors ${index % 2 !== 0 ? 'bg-gray-50' : ''}`}>
                    ...
                    <td className="py-3 px-4 text-gray-800 text-sm">{item.tanggal}</td>
                    <td className="py-3 px-4 text-gray-800 text-sm">{item.karyawanHadir}</td>
                  </tr>
                ))}
              </tbody>
            </table>
